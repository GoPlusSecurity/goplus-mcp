...
exports.registerSuiTokenSecurityTool = registerSuiTokenSecurityTool;
 ...
 */
function analyzeSuiTokenSecurityData(tokenData, tokenAddress) {
    try {
 ...
 */
function registerSuiTokenSecurityTool(server) {
    server.tool('sui_token_security', 'Analyze Sui token security and detect potential risks', {
        contract_addresses: zod_1.z.string().describe('Sui token contract address(es) to analyze, separated by commas for multiple addresses')
        ...
        try {
            ...
            // Make API request
            const response = await axios_1.default.get(`${index_js_1.API_BASE_URL}/sui/token_security`, {
                params: {
