...
export const suiTokenSecurityTool: Tool = {
  name: "suiTokenSecurity",
  description: "Analyze Sui token security and detect potential risks",
  inputSchema: {
    type: "object",
    properties: {
      address: {
  ...
  handler: async (args: any) => {
    try {
      ...
      const response = await axios.get(
        `https://api.gopluslabs.io/api/v1/sui/token_security?addresses=${address}`,
        {
          headers: {
      ...
      if (!data || !data.code || data.code !== 1) {
        throw new Error(data?.message || "Failed to fetch Sui token security data");
      }
      ...
      if (!results || Object.keys(results).length === 0) {
        return {
          content: [{
            ...
            text: JSON.stringify({
              success: false,
              message: "No security data found for this Sui token"
            }, null, 2)
          }]
        };
      }
      ...
      logger.error(`Error in suiTokenSecurity tool: ${error.message}`);
      return {
        content: [{
          type: "text",
          text: JSON.stringify({
