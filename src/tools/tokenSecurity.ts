 ...
 */
function analyzeTokenSecurityData(data: any): any {
  try {
    if (!data || !data.result) {
 ...
 */
function createErrorResponse(tokenAddress: string, errorMessage: string): string {
...
export const tokenSecurityTool: Tool = {
  name: "tokenSecurity",
  description: "Detect security risks of token contracts and identify potential vulnerabilities",
  inputSchema: {
    type: "object",
    properties: {
      chainId: {
  ...
  handler: async (args: any) => {
    try {
      ...
      if (!data || !data.code || data.code !== 1) {
        throw new Error(data?.message || "Failed to fetch token security data");
      }
      ...
      const securityScore = Math.max(0, 100 - (riskCount * 20));
      ...
      console.error("Error in tokenSecurity tool:", error);
      return {
        content: [{
          type: "text",
          text: JSON.stringify({
