 ...
 */
function analyzeSolanaTokenSecurityData(tokenData: any, tokenAddress: string): any {
  try {
    ...
    if (tokenData.dex && tokenData.dex.length > 0) {
      ...
      const highBurnPools = tokenData.dex.filter((d: any) => d.burn_percent > 50);
    ...
    if (tokenData.lp_holders && tokenData.lp_holders.length > 0) {
      const lpInfo = {
        ...
        lockedLp: tokenData.lp_holders.filter((lp: any) => lp.is_locked === 1).length
      };
 ...
 */
export function registerSolanaTokenSecurityTool(server: McpServer): void {
  server.tool(
    'solana_token_security',
    'Analyze Solana token security and detect potential risks',
    {
      contract_addresses: z.string().describe('Solana token contract address(es) to analyze, separated by commas for multiple addresses')
    },
    async ({ contract_addresses }): Promise<CallToolResult> => {
      try {
        ...
        // Make API request
        const response = await axios.get(`${API_BASE_URL}/solana/token_security`, {
          params: {
